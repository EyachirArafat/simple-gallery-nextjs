...
export default function FavoritesPage() {
    // Simulate favorited items (in real app, this would come from database)
    const [favorites, setFavorites] = useState(
        mediaData.slice(0, 4).map((item) => ({
            id: item.id,
            src: item.src,
            title: item.title,
                ...
                {favorites.length > 0 && (
                    <div className="mb-6">
                        <p className="text-sm text-gray-400">
                            <span className="text-white font-medium">{favorites.length}</span> items in your favorites
                        </p>
                    </div>
                )}
