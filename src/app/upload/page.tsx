...
export default function UploadPage() {
  ...
  const handleFiles = (newFiles: File[]) => {
    ...
    if (validFiles.length === 0) {
      alert(
        `Please upload ${mediaType === "photo" ? "image" : "video"} files only.`
      );
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            >
              <Image
                className={`w-8 h-8 mx-auto mb-3 ${
                  mediaType === "photo" ? "text-purple-400" : "text-gray-400"
                }`}
              />
              <p
                className={`font-semibold ${
                  mediaType === "photo" ? "text-white" : "text-gray-300"
                }`}
              >
                Photos
              </p>
            ...
            >
              <Video
                className={`w-8 h-8 mx-auto mb-3 ${
                  mediaType === "video" ? "text-blue-400" : "text-gray-400"
                }`}
              />
              <p
                className={`font-semibold ${
                  mediaType === "video" ? "text-white" : "text-gray-300"
                }`}
              >
                Videos
              </p>
          ...
          {/* Drop Zone */}
          <div
            className={`relative p-8 md:p-12 rounded-2xl border-2 border-dashed transition-all duration-200 ${
              dragActive
                ? "border-purple-500 bg-purple-500/10"
          ...
          >
            {isUploading ? (
              ...
              <>
                ...
                Upload{" "}
                {files.length > 0
                  ? `${files.length} ${mediaType}${files.length > 1 ? "s" : ""}`
                  : "Media"}
              </>
            )}
          </button>
