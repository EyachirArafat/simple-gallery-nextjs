...
export default function Sidebar() {
    ...
    const NavLink = ({ item }: { item: (typeof navItems)[0] }) => {
        ...
        `}
            >
                <Icon
                    className={`w-5 h-5 flex-shrink-0 transition-transform duration-200 group-hover:scale-110 ${isActive ? "text-purple-400" : ""
                        }`}
                />
                <span
                ...
                <div className="flex flex-col h-full p-4">
                    ...
                    <div className="flex items-center justify-between mb-8 px-2">
                        <Link href="/" className="flex items-center gap-3">
                            ...
                            <span
                                className={`font-bold text-xl bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent whitespace-nowrap overflow-hidden transition-all duration-200 ${isCollapsed ? "w-0 opacity-0" : "w-auto opacity-100"
                                    }`}
                            >
                                MediaHub
                            </span>
